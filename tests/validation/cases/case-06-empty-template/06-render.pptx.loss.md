> Summary: 11 lossless, 6 lossy, 5 dropped, 0 annotated.

# Loss manifest — Design system at 12 months: what worked, what didn't, what's next

- Rendered: 2026-06-01T22:27:05
- Renderer: `render-pptx`
- Template: `/sessions/jolly-nifty-cori/mnt/Slide Publisher/plugin/tests/validation/cases/case-06-empty-template/input.pptx`
...
- **slide `three-things-that-worked` / layout_intent** — intent 'three_pillars' substituted to 'claim_with_evidence' (template layout 'Title and Content') via the documented fallback chain.
- **slide `what-the-data-says` / layout_intent** — intent 'metrics' substituted to 'claim_with_evidence' (template layout 'Title and Content') via the documented fallback chain.
- **slide `a-voice-from-the-field` / layout_intent** — intent 'quote' substituted to 'claim_with_evidence' (template layout 'Title and Content') via the documented fallback chain.
- **slide `what-next` / layout_intent** — intent 'three_pillars' substituted to 'claim_with_evidence' (template layout 'Title and Content') via the documented fallback chain.
- **slide `the-callback` / layout_intent** — intent 'callout' substituted to 'claim_with_evidence' (template layout 'Title and Content') via the documented fallback chain.
- **slide `next-steps` / layout_intent** — intent 'callout' substituted to 'claim_with_evidence' (template layout 'Title and Content') via the documented fallback chain.
